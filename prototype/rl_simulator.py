...
class CustomMujocoGymEnv(gym.Env):
    # TODO: add headless?
    metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 60}
    body:ModularRobot
    
    def __init__(self,body, render_mode=None, seed=None, generate_terrain=False):
        """
        Custom MuJoCo environment that uses gymnasium format.

        Args:
            render_mode (str): Either "human" or "rgb_array" for rendering options.
        """
        super().__init__()
        # TODO: decide between body: ModularRobot or genotype
        # ModularRobot from genotype.develop()
        self.body = body

        # create robot scene
        modularRobotScene = ModularRobotScene(terrain=terrains.flat())
        modularRobotScene.add_robot(body)
        # convert ModularRobotScene to Scene
        scene = modularRobotScene.to_simulation_scene()

        # TODO: simulation timestep = ?
        # get model from scene, model is of type mujoco.MjModel
        model, mapping = scene_to_model(
            scene, simulation_timestep=10.0, cast_shadows=False, fast_sim=True
        )
        self.model = model
        self.data = mujoco.MjData(self.model)

        # with control interface position and velocity of hinges can be set
        self.control_interface = ControlInterfaceImpl(
            data=self.data, abstraction_to_mujoco_mapping=mapping
        )
        ...
        # TODO: get sensor data from active hinges
        active_hinge_sensors = active_hinges[0].sensors()
        # ActiveHinge is subclass of Module, which has the method sensors()

        # get joints and hinges ids using xml
        pattern = r'[^"]*jointy_0[^"]*'
        ...
        # hinges are used for action space, observation space
    ...
    def reset(self, seed=None, options=None):
        ...
        # Set the initial state of joints
...
register(
    id="CustomMujoco-v0",
    entry_point=CustomMujocoGymEnv,
